...
export async function fetchBlogs() {
  try {
    ...
    return {
      success: true,
      data: [...data.map((item: any) => ({ ...item.blog, id: item.id, updatedAt: item.updatedAt.toDate() }))],
    };
  } catch (err: any) {
...
export async function fetchBlog({ subDocumentId = "" }) {
  try {
    ...
    return { success: true, data: { ...data.blog, id: data.id, updatedAt: data.updatedAt.toDate() } };
  } catch (err: any) {
